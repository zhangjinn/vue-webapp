<template>
	<div class="app" v-cloak>
		...
		<!--班级-->
		<van-popup v-model="showClass">
			<!-- 班级 -->
			<div class="my-container">
				<!-- 年级列表  radio-->
				<van-radio-group v-model="classValue" @change="changeClass">
					<van-radio :name="item" v-for="(item,index) in classes">
						<span>{{ item }}</span>
					</van-radio>
				</van-radio-group>
		...
		<van-popup v-model="showPlayer">
			...
			<div class="showPlayer">
				...
				<van-radio-group v-model="radio" @change="changeStudent">
					<van-radio :name="item" v-for="(item,index) in students">
	...
	export default {
		...
		created() {
			...
			if (_this.user == undefined || _this.user == null) {
				...
				// 获取选手
				getPerson2( _this.user.user.person).then(function (res) {
					 _this.user_person = res.data;
					 // console.log('_this.user_person 家长信息=',res)
				});
				// 获取相关的选手列表
				_this.getStudentList();
		...
		methods: {
			...
			//获取选手列表
			 getStudentList() {
				let _this = this;
				//用户personId
				let user_person_id = _this.user.user.person;

				 getByUser({	user: user_person_id}).then(function (res) {
				 	// 如果返回的数据 有null 数据 则页面会报错
				 	_this.students = res.data.splice(0,19);
					 console.log(_this.students,'选手列表')
				 })
