<template>
	<div class="topic">
		...
		<template v-if="itemBig">
			<topic-big :id="id" :current-item="currentItem" :part-index="partIndex" :answer-index="answerIndex" :item-index="itemIndex" :child-index="childIndex"
			:question-index="questionIndex"
			v-on:stopCountdown="stopCountdown()" v-on:countDownAgain="countDowntime()"></topic-big>
		</template>
		<template v-if="itemSmall">
			<topic-small :id="id" :current-item="currentItem" :part-index="partIndex" :answer-index="answerIndex" :item-index="itemIndex"
			:question-index="questionIndex"
		...
		<div class="topicBtn">
			...
			<span :class="questionIndex == (questionCount-1)&&(currentItem.son.length>0 ? childIndex+1==currentItem.son.length : childIndex==0) ? 'next default':'next'" @click="next()">下一题</span>
		</div>
	</div>
</template>
...
<script>
	...
	export default {
		...
		props:["currentData","partIndex","questionIndex","questionCount"],
		components:{
			TopicBig,
			TopicSmall
		},
